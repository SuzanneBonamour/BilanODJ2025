...
plot_ODJ_STOC <- function(sp_vec, 
  ...
  plot_one_species <- function(sp) {
    ...
    pourc <- exp(dataLT_sp$estimate*(yearValues-min(yearValues)))*yRef
    pourc_inf <- exp(dataLT_sp$infIC*(yearValues-min(yearValues)))*yRef
    pourc_sup <- exp(dataLT_sp$supIC*(yearValues-min(yearValues)))*yRef

    dataPlot <- data.frame(
      year = yearValues,
      estimate = pourc,
      estimate_inf = pourc_inf,
      estimate_sup = pourc_sup,
      infIC = NA,
      supIC = NA,
      group = "LT",
      rep = 1
    ...
    pourc_STOC <- exp(dataLT_sp_STOC$estimate*(yearValues_STOC-min(yearValues_STOC)))*yRef
    pourc_inf_STOC <- exp(dataLT_sp_STOC$infIC*(yearValues_STOC-min(yearValues_STOC)))*yRef
    pourc_sup_STOC <- exp(dataLT_sp_STOC$supIC*(yearValues_STOC-min(yearValues_STOC)))*yRef
    
    dataPlot_STOC <- data.frame(
      year = yearValues_STOC,
      estimate = pourc_STOC,
      estimate_inf = pourc_inf_STOC,
      estimate_sup = pourc_sup_STOC,
      infIC = NA,
      supIC = NA,
      group = "LT",
      rep = 1
    ...
    p <- ggplot() +
      ...
      geom_ribbon(data = dataPlot, aes(x = year, y = estimate, 
                                       ymin = estimate_inf, ymax = estimate_sup), fill = "black", alpha = 0.1) + 
      geom_ribbon(data = dataPlot_STOC, aes(x = year, y = estimate, 
                                       ymin = estimate_inf, ymax = estimate_sup), fill = color_stoc, alpha = 0.1) +
...
#   # Convertir en ASCII et nettoyer
#   safe_name <- iconv(as.character(sp), from = "UTF-8", to = "ASCII//TRANSLIT")
#   safe_name <- gsub("[^[:alnum:]_]", "_", safe_name)
#   stopifnot(length(safe_name) == 1, nchar(safe_name) > 0)
#   
#   if (!dir.exists(out_dir)) dir.create(out_dir, recursive = TRUE)
#   file_base <- file.path(out_dir, safe_name)
#   
#   # Sauvegarde PNG
#   ggsave(paste0(file_base, ".png"), plot = p, width = 7, height = 5, dpi = 300)
#   
#   # Si vraiment besoin de SVG, utiliser le même safe_name
#   ggsave(paste0(file_base, ".svg"), plot = p, width = 7, height = 5)
# }
...
# --- Sauvegarde si demandé ---
# if (!is.null(p)) {
#   # Convertir le nom de l'espèce en ASCII et nettoyer
#   safe_name <- iconv(as.character(sp), from = "UTF-8", to = "ASCII//TRANSLIT")
#   safe_name <- gsub("[^[:alnum:]_]", "_", safe_name)
#   
#   # Vérifier que safe_name est valide
#   if (length(safe_name) != 1 || nchar(safe_name) == 0 || is.na(safe_name)) {
#     safe_name <- paste0("plot_", as.integer(Sys.time()))
#   }
#   
...
#   
#   # Sauvegarde PNG
#   ggsave(filename = paste0(file_base, ".png"), plot = p, width = 7, height = 5, dpi = 300)
#   
#   # Sauvegarde SVG
#   ggsave(filename = paste0(file_base, ".svg"), plot = p, width = 7, height = 5)
# }
