...
plot_ODJ_STOC <- function(sp_vec, 
  ...
  plot_one_species <- function(sp) {
    ...
    # estLT <- (dataLT_sp$estimate*abond) * (yearValues - min(yearValues))
    # estLT <- estLT - mean(estLT)
    tt <- exp(dataLT_sp$estimate*(yearValues-min(yearValues)))*yRef
    
    if ((dataLT_sp$estimate <= 0)) {
      estLT_ok <- exp(estLT) + (abond - exp(estLT[1]))} else{
      estLT_ok <- exp(estLT) + (abond - exp(estLT[1]))}
    
    # val2014 <- estLT_ok[which(yearValues == 2014)]
    # estLT_index <- estLT_ok / val2014 * 100

    dataPlot <- data.frame(
      year = yearValues,
      # estimate = estLT_index,
      estimate = tt,
      infIC = NA,
      supIC = NA,
      group = "LT",
    ...
    pourc <- exp(dataLT_sp_STOC$estimate*(yearValues_STOC-min(yearValues_STOC)))*yRef
