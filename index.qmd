...
verdier_dt <- comparaison_ODJ_STOC %>% 
  filter(species == "Verdier d'Europe") %>% 
  select(species, minYear, maxYear, estimate, pval, tendance_bilan)
```
...
verdier_info_ODJ_STOC <- data.frame(
  slope = c(verdier_dt$estimate),          # pentes
  intercept = c(0, 0),         # ordonnées à l'origine
  line = c("ODJ", "STOC")  # étiquettes
)

verdier_ODJ_STOC_plot <- ggplot(verdier_info_ODJ_STOC, aes(color = line)) +
  geom_abline(aes(slope = slope, intercept = intercept), 
              size = 1, color = c("darkgrey", "darkgreen")) +
  xlim(-1, 1) + ylim(-1, 1) + 
  theme_trends(base_size=12, color=NA)+
  labs(x = "Temps", y = "Effectif relatifs", color = "Lignes") +
...
verdier_dt <- comparaison_ODJ_SHOC %>% 
  filter(species == "Verdier d'Europe") %>% 
  select(species, minYear, maxYear, estimate, pval, tendance_bilan)
```
