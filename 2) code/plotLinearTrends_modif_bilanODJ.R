...
#' 
# plotLinearTrends <- function(speciesList, data, dataLongTerm, dataYearlyVariations, dataShortTerm = F,
#                       dataName = NULL, distribution, plotGamm = F, plotST = F,
#                       uncertainty = F, N = 100, weight = TRUE, save = TRUE, path = ""){

# library(readr)
# dataLongTerm <- read_delim("C:/Users/suzanne.bonamour/OneDrive - LPO/2) Data/7) ODJ bilan/1) Analyses/BilanODJ2025/1) data/longTermTrends.csv", 
...
dataLongTerm <- read_csv("C:/Users/suzanne.bonamour/OneDrive - LPO/2) Data/7) ODJ bilan/1) Analyses/BilanODJ2025/1) data/longTermTrends.prn")

dataLongTerm$species <- dataLongTerm$french_name

speciesList <- c("Verdier d'Europe", "Rougegorge familier")
    ...
    # }else{
      ...
      # if(all(dataPlot$estimate < 10^10)){
        ...
        plot = plot +
          ...
          ggplot2::geom_line(data = dataPlot[dataPlot$group == "LT",], ggplot2::aes(x = year, y = estimate),
        ...
        # Color values
        # colValues = colValues_guide = c("#429EBD", "#F8C457")
        colValues = colValues_guide = c("#429EBD")
        # Labels for each color
        # colLabels = c("legendLT", "legendVar") 
        colLabels = c("legendLT") 
        # Other characteristics
        linetype = c(1, 0) ; shape = c(NA, 18)
        size = c(1.1, 3) ; alpha = c(1, 1)
        ...
        plot <-  plot + 
          ...
          ggplot2::scale_x_continuous(name = x_axisName, breaks = yearValues, limits = c(dataLT_sp$minYear - 1, dataLT_sp$maxYear +1)) + # change year labels on x axis for winter season for SHOC)
